  ...
  s"POST " + normalRoutePath - {
        ...
        "when the session contains no data for page" in {
          ...
          WsTestClient.withClient { client =>
            val result = createClientRequestPOST(
              ...
              Json.obj("value" -> Json.toJson(YesRegister.toString))
            )
            ...
            whenReady(result) { res =>
              ...
              userAnswersAfterPOST.get.address.get mustBe UkAddress(
    ...
    s"when the user selects $YesNewAddress" - {
      "should update the session with the new value and redirect to the ALF" - {
        "when the session contains no data for page" in {
          build.commonPrecondition.alf.getSuccessResponseFromALFInit("foo")
          ...
          WsTestClient.withClient { client =>
            val result = createClientRequestPOST(
              client,
              baseUrl + normalRoutePath,
              Json.obj("value" -> Json.toJson(YesNewAddress.toString))
            )
      ...
      "should NOT update the session with the new value and redirect to the Auth sign out controller" in {
        ...
        WsTestClient.withClient { client =>
          val result = createClientRequestPOST(
            client,
            baseUrl + normalRoutePath,
            Json.obj("value" -> Json.toJson(No.toString))
          )
