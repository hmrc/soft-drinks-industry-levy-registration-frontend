      ...
      "should return OK and render the PackagingSiteDetails page with no radio items selected" in {
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result1) { res =>
            ...
            summaryRow.text() must include(
              (packagingSite1.address.lines :+ packagingSite1.address.postCode).mkString(", ")
            )
        ...
        s"should return OK and render the page with " + key + " radio unchecked" in {
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result1) { res =>
              ...
              val summaryRow      = summaryListRows.get(0)
              summaryRow.text() must include(
                packagingSite1.address.lines.mkString(", ") + s", ${packagingSite1.address.postCode}"
              )
              val radioInputs = page.getElementsByClass("govuk-radios__input")
              radioInputs.size() mustBe 2
              radioInputs.get(0).attr("value") mustBe "true"
            ...
            WsTestClient.withClient { client =>
              ...
              whenReady(result) { res =>
                ...
                val dataStoredForPage =
                  getAnswers(userAnswers.id).fold[Option[Boolean]](None)(_.get(PackagingSiteDetailsPage))
