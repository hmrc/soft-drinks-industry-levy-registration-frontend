  ...
  modeWithPaths.foreach { case (mode, path) =>
        ...
        "should return OK and render the PackagingSiteName page with data populated" in {
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result1) { res =>
              ...
              inputFields
                .get(0)
                .getElementById("packagingSiteName")
                .attr("value") mustBe packagingSiteName.packagingSiteName
            }
          }
        }
      }
    ...
    "POST " + path - {
          ...
          "and the userAnswers contains alfResponseWithLookupState for the reference number" in {
            ...
            WsTestClient.withClient { client =>
              ...
              whenReady(result1) { res =>
                ...
                updatedUserAnswer.packagingSiteList mustBe Map(
                  ref -> Site(ukAddress, None, packagingSiteNameDiff.packagingSiteName, None)
                )
              }
            }
          }
      ...
      "should return 400 with required error" - {
        ...
        packagingSiteNameMap.zipWithIndex.foreach { case ((fieldName, _), _) =>
