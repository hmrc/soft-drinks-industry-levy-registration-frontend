  ...
  ".set" - {
    ...
    "correctly encrypt the records data" in {
      ...
      val businessAddressDecrypted   =
        Json
          .fromJson[Option[UkAddress]](
            Json.parse(
              encryption.crypto.decrypt((resultParsedToJson \ "address").as[EncryptedValue], userAnswersBefore.id)
            )
      ...
      val warehouseListDecrypted     = {
        ...
        json.map(warehouse =>
          warehouse._1 -> Json.parse(encryption.crypto.decrypt(warehouse._2, userAnswersBefore.id)).as[Warehouse]
        )
      }
