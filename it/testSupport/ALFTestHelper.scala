...
object ALFTestHelper {
  ...
  def requestedBodyMatchesExpected(wireMockServer: WireMockServer, bodyExpected: JourneyConfig): Boolean = {
    val requestMadeToAlf                  =
      wireMockServer.getAllServeEvents.asScala.toList.map(_.getRequest).filter(_.getUrl.contains("/api/init")).head
    ...
    val continueUrlSentMatches =
      if (bodyExpected.options.continueUrl.nonEmpty) {
        requestMadeToAlf.getBodyAsString.contains(
          bodyExpected.options.continueUrl.substring(0, bodyExpected.options.continueUrl.indexOf("/"))
        )
      } else { true }
