...
case class SdilBackendStub()(implicit builder: PreconditionBuilder) {
  ...
  val aSubscriptionWithDeRegDate = aSubscription.copy(deregDate = Some(LocalDate.of(2022, 2, 11)))

  def retrieveSubscription(identifier: String, refNum: String) = {
    stubFor(
      get(urlPathMatching(s"/subscription/$identifier/$refNum"))
        .willReturn(ok(Json.toJson(aSubscription).toString()))
    )
    builder
  }
  ...
  def retrieveRosm(utr: String, rosmReg: RosmRegistration = rosmRegistration) = {
    stubFor(
      get(urlPathMatching(s"/rosm-registration/lookup/$utr"))
        .willReturn(ok(Json.toJson(rosmReg).toString()))
    )
    builder
  }
  ...
  def createSubscription(utr: String) = {
    stubFor(
      post(urlEqualTo(s"/subscription/utr/$utr/safeid"))
        .willReturn(ok())
    )
    builder
  }
