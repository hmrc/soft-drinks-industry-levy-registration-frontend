...
class StartDateControllerISpec extends ControllerITTestHelper {
  ...
  val normalRoutePath = "/startDate"
  val checkRoutePath = "/changeStartDate"
        ...
        "when the session contains no data for page" in {
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              ...
              res.header(HeaderNames.LOCATION) mustBe Some(PackAtBusinessAddress.url)
              val dataStoredForPage = getAnswers(sdilNumber).fold[Option[LocalDate]](None)(_.get(StartDatePage))
