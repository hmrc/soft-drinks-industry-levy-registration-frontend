...
import models.TestRadio

class TestRadioControllerISpec extends ControllerITTestHelper {

  val normalRoutePath = "/testRadio"
  val checkRoutePath = "/changeTestRadio"

  "GET " + normalRoutePath - {
    "when the userAnswers contains no data" - {
      "should return OK and render the TestRadio page with no data populated" in {
        given
          .commonPrecondition
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result1) { res =>
            ...
            page.title must include(Messages("testRadio" + ".title"))
            val radioInputs = page.getElementsByClass("govuk-radios__input")
            radioInputs.size() mustBe TestRadio.values.size

            TestRadio.values.zipWithIndex.foreach { case (radio1, index1) =>
    ...
    TestRadio.values.zipWithIndex.foreach { case (radio, index) =>
      s"when the userAnswers contains data for the page with " + radio.toString + " selected" - {
        s"should return OK and render the page with " + radio.toString + " radio checked" in {
          ...
          val userAnswers = emptyUserAnswers.set(TestRadioPage, radio).success.value
    ...
    testOtherSuccessUserTypes(baseUrl + normalRoutePath, Messages("testRadio" + ".title"))
  ...
  s"GET " + checkRoutePath - {
    ...
    testOtherSuccessUserTypes(baseUrl + checkRoutePath, Messages("testRadio" + ".title"))
  ...
  s"POST " + normalRoutePath - {
    TestRadio.values.foreach { case radio =>
          ...
          "when the session contains no data for page" in {
            ...
            WsTestClient.withClient { client =>
              ...
              whenReady(result) { res =>
                ...
                res.header(HeaderNames.LOCATION) mustBe Some(routes.TestMQController.onPageLoad.url)
                val dataStoredForPage = getAnswers(sdilNumber).fold[Option[TestRadio]](None)(_.get(TestRadioPage))
          ...
          "when the session already contains data for page" in {
            ...
            WsTestClient.withClient { client =>
              ...
              whenReady(result) { res =>
                res.status mustBe 303
                res.header(HeaderNames.LOCATION) mustBe Some(routes.TestMQController.onPageLoad.url)
                val dataStoredForPage = getAnswers(userAnswers.id).fold[Option[TestRadio]](None)(_.get(TestRadioPage))
                dataStoredForPage.nonEmpty mustBe true
                dataStoredForPage.get mustBe radio
              }
            }
      ...
      "should return 400 with required error" in {
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result) { res =>
            ...
            val page = Jsoup.parse(res.body)
            page.title must include("Error: " + Messages("testRadio" + ".title"))
            val errorSummary = page.getElementsByClass("govuk-list govuk-error-summary__list")
              .first()
            errorSummary
              .select("a")
              .attr("href") mustBe "#value_0"
            errorSummary.text() mustBe Messages("testRadio" + ".error.required")
          }
        }
      }
    }
