...
class Navigator @Inject() () {
  ...
  def nextPage(page: Page, mode: Mode, userAnswers: UserAnswers, previousAnswer: Option[String] = None): Call =
    mode match {
      case NormalMode =>
  ...
  private val checkRouteMap: Page => UserAnswers => Option[String] => Call = {
    ...
    case HowManyLitresGloballyPage      =>
      userAnswers => previousAnswer => navigationForHowManyLitresGloballyCheckMode(userAnswers, previousAnswer)
    case _                              => _ => _ => routes.CheckYourAnswersController.onPageLoad
  }

  private def navigationForHowManyLitresGloballyCheckMode(
  ...
  private def operatesPackagingSite(userAnswers: UserAnswers): Boolean =
    UserTypeCheck.operatesPackagingSite(userAnswers)
