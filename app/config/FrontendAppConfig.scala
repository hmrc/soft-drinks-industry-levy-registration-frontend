...
class FrontendAppConfig @Inject() (servicesConfig: ServicesConfig, configuration: Configuration) {
  ...
  val sdilFoundingDate: LocalDate = LocalDate.of(
    servicesConfig.getInt("sdilFoundingDate.year"),
    servicesConfig.getInt("sdilFoundingDate.month"),
    servicesConfig.getInt("sdilFoundingDate.day")
  )
  ...
  private val accessibilityHost: String = servicesConfig.getConfString(
    ...
    throw new Exception("missing config accessibility-statement.host")
  )
  def accessibilityFooterUrl            =
    s"$accessibilityHost/accessibility-statement/soft-drinks-industry-levy-registration-frontend"
