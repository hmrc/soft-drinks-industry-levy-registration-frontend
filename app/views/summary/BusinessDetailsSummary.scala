...
object BusinessDetailsSummary {
  ...
  def headingAndSummary(
    ...
    val summaryList      = SummaryListViewModel(rows)
    heading -> summaryList
  }

  private def rowsWithActions(
    subscription: Subscription,
    formattedAddress: Content,
    numberOfLitres: HowManyLitresGlobally
  )(implicit messages: Messages): Seq[SummaryListRow] = {
    val businessAddressAction = getAction("businessAddress", routes.VerifyController.onPageLoad(CheckMode).url)
    val litresGloballyAction  =
      getAction("howManyLitresGlobally", routes.HowManyLitresGloballyController.onPageLoad(CheckMode).url)
    Seq(
      ...
      createSummaryListItem(
        "litresGlobally",
        Text(messages(s"howManyLitresGlobally.${numberOfLitres.toString}")),
        Some(litresGloballyAction)
      )
    )
  }

  private def rowsWithNoActions(
    subscription: Subscription,
    formattedAddress: Content,
    numberOfLitres: HowManyLitresGlobally
  )(implicit messages: Messages): Seq[SummaryListRow] =
  ...
  private def createSummaryListItem(fieldName: String, content: Content, actions: Option[Actions] = None)(implicit
    messages: Messages
  ): SummaryListRow =
    SummaryListRow(
