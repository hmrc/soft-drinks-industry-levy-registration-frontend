...
object PackagingSiteDetailsSummary {
  ...
  def row2(packagingSiteList: Map[String, Site], mode: Mode)(implicit messages: Messages): List[SummaryListRow] =
    packagingSiteList.map { site =>
      SummaryListRow(
        key = Key(
          content = HtmlContent(AddressFormattingHelper.addressFormatting(site._2.address, site._2.tradingName)),
          ...
          Some(
            Actions(
              ...
              Seq(
                ActionItemViewModel(
                  "site.remove",
                  routes.RemovePackagingSiteDetailsController.onPageLoad(mode, site._1).url
                )
                  .withVisuallyHiddenText(
                    messages("packagingSiteDetails.remove.hidden", site._2.tradingName, site._2.address.lines.head)
                  )
              )
            )
          )
