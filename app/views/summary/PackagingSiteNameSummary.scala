...
object PackagingSiteNameSummary  {
    ...
    answers.get(PackagingSiteNamePage).map {
      ...
      val value = HtmlFormat.escape(answer.PackagingSiteName).toString
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.PackagingSiteNameController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("packagingSiteName.change.hidden"))
          )
        )
    }
}
