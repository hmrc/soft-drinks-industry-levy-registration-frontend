...
object ContactDetailsSummary {
  ...
  def headingAndSummary(subscription: Subscription, isCheckAnswers: Boolean = true)(implicit
    messages: Messages
  ): (String, SummaryList) = {
    val contactDetails = subscription.contact
    val heading        = messages("contactDetails.title")
    val summaryList    = SummaryListViewModel(rows(contactDetails, isCheckAnswers))
    heading -> summaryList
  }
