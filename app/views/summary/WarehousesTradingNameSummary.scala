...
object WarehousesTradingNameSummary  {
    ...
    answers.get(WarehousesTradingNamePage).map {
      ...
      val value = HtmlFormat.escape(answer.warehouseTradingName).toString
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.WarehousesTradingNameController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("warehousesTradingName.change.hidden"))
          )
        )
    }
}
