...
object UKSitesSummary {
  ...
  private def getPackAtBusinessAddressRow(subscription: Subscription, isCheckAnswers: Boolean)(implicit
    messages: Messages
  ): SummaryListRow =
  ...
  private def getAskSecondaryWarehouseRow(subscription: Subscription, isCheckAnswers: Boolean)(implicit
    messages: Messages
  ): SummaryListRow =
    SummaryListRowViewModel(
  ...
  def getHeadingAndSummary(
    ...
    val optSummaryList =
      (subscription.productionSites.nonEmpty, subscription.isVoluntary(howManyLitresGlobally)) match {
        case (true, false)  =>
          Option(
        ...
        case (true, true)   =>
          Option(SummaryListViewModel(Seq(getPackAtBusinessAddressRow(subscription, isCheckAnswers))))
        case (false, false) =>
          Option(SummaryListViewModel(Seq(getAskSecondaryWarehouseRow(subscription, isCheckAnswers))))
        case _              => None
      }
