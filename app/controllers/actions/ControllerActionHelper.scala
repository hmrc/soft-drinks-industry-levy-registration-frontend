...
trait ControllerActionHelper {
  ...
  def dataRequiredForRequiresBusinessDetailsAction(implicit
    ec: ExecutionContext
  ): ActionRefiner[OptionalDataRequest, DataRequestForEnterBusinessDetails] =
    new ActionRefiner[OptionalDataRequest, DataRequestForEnterBusinessDetails] {
      override protected def refine[A](
        request: OptionalDataRequest[A]
      ): Future[Either[Result, DataRequestForEnterBusinessDetails[A]]] = {
        ...
        request.userAnswers match {
          ...
          case Some(userAnswers) if canAccessEnterBusinessDetails(userAnswers) =>
            Future.successful(
              Right(
                DataRequestForEnterBusinessDetails(
                  request = request.request,
                  internalId = request.internalId,
                  hasCTEnrolment = request.hasCTEnrolment,
                  authUtr = request.authUtr,
  ...
  def dataRequiredForUserWhoCannotRegisterAction(
    registerState: RegisterState
  )(implicit ec: ExecutionContext): ActionRefiner[OptionalDataRequest, DataRequest] =
  ...
  def dataRequiredForUserWhoHasSubmittedApplicationAction(implicit
    ec: ExecutionContext
  ): ActionRefiner[OptionalDataRequest, DataRequestForApplicationSubmitted] =
    new ActionRefiner[OptionalDataRequest, DataRequestForApplicationSubmitted] {
      override protected def refine[A](
        request: OptionalDataRequest[A]
      ): Future[Either[Result, DataRequestForApplicationSubmitted[A]]] = {
        implicit val hc: HeaderCarrier = HeaderCarrierConverter.fromRequestAndSession(request, request.session)
        request.userAnswers match {
          case Some(userAnswers) if userAnswers.submittedOn.isDefined =>
            getRosmData(userAnswers, request).map {
              case Right(rosmWithUtr) =>
                Right(
                  DataRequestForApplicationSubmitted(
                    request,
                    request.internalId,
                    userAnswers,
                    rosmWithUtr,
                    userAnswers.submittedOn.get
                  )
                )
              case Left(result)       => Left(result)
            }
  ...
  def dataRequiredForEnterTradingNameAction(addressLookupState: AddressLookupState, ref: String, mode: Mode)(implicit
    ec: ExecutionContext
  ): ActionRefiner[DataRequest, DataRequestForEnterTradingName] =
    new ActionRefiner[DataRequest, DataRequestForEnterTradingName] {
      override protected def refine[A](
        request: DataRequest[A]
      ): Future[Either[Result, DataRequestForEnterTradingName[A]]] =
        Future.successful {
          ...
          userAnswers.alfResponseForLookupState match {
            case Some(alfResponseForLookupState)
              ...
              Right(
                DataRequestForEnterTradingName(
                  request.request,
                  request.internalId,
                  request.hasCTEnrolment,
                  request.authUtr,
                  request.userAnswers,
                  alfResponseForLookupState.address,
                  None
                )
              )
      ...
      private def handleUserWhoCannotEnterTradingNameForWarehouse[A](
        userAnswers: UserAnswers,
        request: DataRequest[A]
      ): Either[Result, DataRequestForEnterTradingName[A]] =
          ...
          case Some(warehouse)                            =>
            Right(
              DataRequestForEnterTradingName(
                ...
                request.userAnswers,
                warehouse.address,
                Some(warehouse.tradingName)
              )
            )
          case None if userAnswers.warehouseList.nonEmpty =>
            Left(Redirect(routes.WarehouseDetailsController.onPageLoad(mode)))
          ...
          case Some(packagingSite)                            =>
            Right(
              DataRequestForEnterTradingName(
                ...
                request.authUtr,
                request.userAnswers,
                packagingSite.address,
                Some(packagingSite.tradingName)
              )
            )
          case None if userAnswers.packagingSiteList.nonEmpty =>
            Left(Redirect(routes.PackagingSiteDetailsController.onPageLoad(mode)))
  ...
  def getRosmData[A](userAnswers: UserAnswers, request: OptionalDataRequest[A])(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Either[Result, RosmWithUtr]] =
