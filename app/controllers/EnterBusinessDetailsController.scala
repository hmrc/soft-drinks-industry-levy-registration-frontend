...
class EnterBusinessDetailsController @Inject() (
  ...
  def onPageLoad: Action[AnyContent] = controllerActions.withRequiresBusinessDetailsAction.async { implicit request =>
  ...
  def onSubmit: Action[AnyContent] = controllerActions.withRequiresBusinessDetailsAction.async { implicit request =>
    form
      .bindFromRequest()
      .fold(
        ...
        identify =>
          if (sameBusinessDetailsEntered(identify, request.userAnswers)) {
            ...
            registrationOrchestrator
              .checkEnteredBusinessDetailsAreValidAndUpdateUserAnswers(identify, request.internalId)
              .value flatMap {
              case Right(state)                                                                              =>
              ...
              case Left(error) if List(NoROSMRegistration, EnteredBusinessDetailsDoNotMatch).contains(error) =>
                genericLogger.logger.error(
                  s"${getClass.getName} - ${request.userAnswers.id} - no rosm registration or business details don't match"
                )
                Future.successful(
                  BadRequest(view(form.fill(identify).withError("utr", "enterBusinessDetails.no-record.utr")))
                )
              case Left(error)                                                                               =>
                genericLogger.logger
                  .error(s"${getClass.getName} - ${request.userAnswers.id} - ${error.getClass.getName} error returned")
  ...
  private def sameBusinessDetailsEntered(identify: Identify, userAnswers: UserAnswers): Boolean =
    userAnswers.get(EnterBusinessDetailsPage).fold(false) { businessDetails =>
      val postcodesMatch = identify.postcode
        .replaceAll(" ", "")
