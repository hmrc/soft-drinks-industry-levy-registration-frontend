...
object ModelEncryption {
  ...
  def encryptUserAnswers(userAnswers: UserAnswers)(implicit encryption: Encryption): (
    ...
    (
      ...
      userAnswers.alfResponseForLookupState.map(alfResponseForLookupState =>
        encryption.crypto.encrypt(Json.toJson(alfResponseForLookupState).toString(), userAnswers.id)
      ),
  ...
  def decryptUserAnswers(
    ...
    UserAnswers(
      ...
      registerState = registerState,
      address = Json
        .fromJson[Option[UkAddress]](Json.parse(encryption.crypto.decrypt(address, id)))(using
          Reads.optionWithNull[UkAddress]
        )
        .get,
      packagingSiteList =
        packagingSiteList.map(site => site._1 -> Json.parse(encryption.crypto.decrypt(site._2, id)).as[Site]),
      warehouseList = warehouseList.map(warehouse =>
        warehouse._1 -> Json.parse(encryption.crypto.decrypt(warehouse._2, id)).as[Warehouse]
      ),
      alfResponseForLookupState = alfResponseForLookupState
        .map(alfRespWithState =>
          Json.parse(encryption.crypto.decrypt(alfRespWithState, id)).as[AddressResponseForLookupState]
        ),
