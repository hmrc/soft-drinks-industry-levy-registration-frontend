...
class RegistrationOrchestrator @Inject() (
  ...
  def handleRegistrationRequest(implicit
    ...
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): RegistrationResult[UserAnswers] = {
    ...
    for {
      ...
      ua             <-
        EitherT.right[RegistrationErrors](
          Future.successful(
  ...
  def checkEnteredBusinessDetailsAreValidAndUpdateUserAnswers(identify: Identify, internalId: String)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): RegistrationResult[RegisterState] =
    for {
  ...
  private def determineRegisterStateForNoneRegisteredUsers(utr: String, internalId: String)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): RegistrationResult[RegisterState] = EitherT {
    val subStatus = for {
  ...
  def createSubscriptionAndUpdateUserAnswers(implicit
    request: DataRequest[AnyContent],
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): RegistrationResult[Unit] = {
    ...
    for {
      createdSubscriptionAndAmountProducedGlobally <-
        EitherT(Future(getSubscriptionAndHowManyLitresGlobally(updatedUserAnswers, request.rosmWithUtr)))
      created                                      <- sdilConnector.createSubscription(
                                                        createdSubscriptionAndAmountProducedGlobally.subscription,
                                                        request.rosmWithUtr.rosmRegistration.safeId
                                                      )
      _                                            <- sessionService.set(updatedUserAnswers)
      _                                            <- EitherT.right(
                                                        sdilSessionCache.save[CreatedSubscriptionAndAmountProducedGlobally](
                                                          internalId,
  ...
  ): Either[RegistrationErrors, CreatedSubscriptionAndAmountProducedGlobally] =
    userAnswers
  ...
  def getRegistrationStateFromSubscriptionStatus(
    subStatus: SubscriptionStatus,
    utrEntered: Boolean = false
  ): RegisterState =
    subStatus match {
