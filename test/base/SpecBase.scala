...
trait SpecBase
  ...
  lazy val application: Application                      =
    applicationBuilder(userAnswers = None, rosmRegistration = rosmRegistration).build()
  ...
  val utr                                                = "1234567891"
  val rosmRegistration: RosmWithUtr                      = RosmWithUtr(
    utr,
    RosmRegistration(
      safeId = "safeid",
  ...
  protected def applicationBuilder(
    ...
    rosmRegistration: RosmWithUtr = rosmRegistration
  ...
  val packingSiteAddress45Characters =
    Site(UkAddress(List("29 Station Pl.", "The Railyard", "Cambridge"), "CB1 2FP"), None, aTradingName, None)

  val packingSiteAddress47Characters =
    Site(UkAddress(List("29 Station Place", "The Railyard", "Cambridge"), "CB1 2FP"), Some("10"), aTradingName, None)

  val packingSiteAddress49Characters =
    Site(UkAddress(List("29 Station PlaceDr", "The Railyard", "Cambridge"), "CB1 2FP"), None, aTradingName, None)

  def packagingSiteListWith3 = Map(
    ("12345678", packingSiteAddress45Characters),
    ("23456789", packingSiteAddress47Characters),
    ("34567890", packingSiteAddress49Characters)
  )

  def warehouse1         =
    Warehouse("Warehouse One", UkAddress(List("29 Station Place", "The Railyard", "Cambridge"), "CB1 2FP"))
  ...
  val userDetailsWithSetMethodsReturningFailure: UserAnswers =
    new UserAnswers("sdilId", RegisterState.RegisterWithAuthUTR) {
      override def set[A](page: Settable[A], value: A)(implicit writes: Writes[A]): Try[UserAnswers] =
        Failure[UserAnswers](new Exception(""))

      override def setAndRemoveLitresIfReq(
        page: Settable[Boolean],
        litresPage: Settable[LitresInBands],
        value: Boolean
      )(implicit writes: Writes[Boolean]): Try[UserAnswers] = Failure[UserAnswers](new Exception(""))
    }
}
