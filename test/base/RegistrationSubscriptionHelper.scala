...
trait RegistrationSubscriptionHelper extends SpecBase {
  ...
  val subscriptionAllFieldsPresent = Subscription(
    ...
    userAnswerDate,
    Seq(PackagingSite1),
    Seq(Site.fromWarehouse(warehouse)),
    contact
  )

  val userAnswerTwoWarehouses: UserAnswers =
    UserAnswers(sdilNumber, RegisterState.RegisterWithAuthUTR, Json.obj(), warehouseList = warehouses)

  def userAnswersWithPagesAcrossAllProducerTypes(
    orgType: OrganisationType,
    litresGlobally: HowManyLitresGlobally,
    litresPopulated: Boolean = false
  ): UserAnswers =
    emptyUserAnswers
      ...
      .success
      .value
      .set(OrganisationTypePage, orgType)
      .success
      .value
      .set(HowManyLitresGloballyPage, litresGlobally)
      .success
      .value
      .set(ContractPackingPage, litresPopulated)
      .success
      .value
      .set(ImportsPage, litresPopulated)
      .success
      .value
  ...
  def userAnswersWithPagesAcrossAllProducerTypesWithLitres(
    orgType: OrganisationType,
    litresGlobally: HowManyLitresGlobally
  ): UserAnswers =
    userAnswersWithPagesAcrossAllProducerTypes(orgType, litresGlobally, true)
      ...
      .set(HowManyContractPackingPage, litresInBands)
      .success
      .value
      .set(HowManyImportsPage, litresInBands)
      .success
      .value
  ...
  def userAnswersLargeProducerIncludingLitres(orgType: OrganisationType): UserAnswers =
    userAnswersWithPagesAcrossAllProducerTypesWithLitres(orgType, HowManyLitresGlobally.Large)
      ...
      .success
      .value
      .set(HowManyOperatePackagingSitesPage, litresInBands)
      .success
      .value
  ...
  def getCompletedUserAnswers(
    orgType: OrganisationType,
    litresGlobally: HowManyLitresGlobally,
    litresPopulated: Boolean
  ): UserAnswers = litresGlobally match {
  ...
  def activityWithLitres(litresGlobally: HowManyLitresGlobally): Activity = {
    ...
    Activity(
      ...
      Some(litreage),
      Some(litreage),
      if (litresGlobally == HowManyLitresGlobally.Small) { Some(Litreage(1, 1)) }
      else {
        None
      },
  ...
  def generateSubscription(
    orgType: OrganisationType = OrganisationType.LimitedCompany,
    ...
    Subscription(
      ...
      orgType.enumNum,
      address,
      if (allFieldsPopulated) { activityWithLitres(litresGlobally) }
      else { activityWithNoLitres(litresGlobally) },
      userAnswerDate,
      if (allFieldsPopulated) { Seq(PackagingSite1) }
      else { Seq.empty },
      if (allFieldsPopulated) { Seq(Site.fromWarehouse(warehouse)) }
      else { Seq.empty },
      contact
    )
