...
class RemovePackagingSiteDetailsControllerSpec extends SpecBase with MockitoSugar with LoggerHelper {
  ...
  "RemovePackagingSiteDetails Controller" - {
    def commonAssertionsForPageLoad(addressToBeDisplayed: Html, page: String, ref: String): Assertion = {
      ...
      doc.getElementsByTag("form").attr("action") mustBe routes.RemovePackagingSiteDetailsController
        .onSubmit(NormalMode, ref)
        .url
    }
      ...
      running(application) {
        withCaptureOfLoggingFrom(application.injector.instanceOf[GenericLogger].logger) { events =>
          ...
          result.header.headers
            .get(LOCATION)
            .mustEqual(Some(routes.PackagingSiteDetailsController.onPageLoad(NormalMode).url))
        }
      }
    }
    ...
    "must redirect to site details page when 1 site exists in list on load" in {
      ...
      val application                      =
        applicationBuilder(userAnswers = Some(emptyUserAnswers.copy(packagingSiteList = packagingSite))).build()
    ...
    "must redirect to site details page when loaded but no site ref exists in list on load" in {
      ...
      running(application) {
        withCaptureOfLoggingFrom(application.injector.instanceOf[GenericLogger].logger) { events =>
          val request =
            FakeRequest(GET, routes.RemovePackagingSiteDetailsController.onPageLoad(NormalMode, wrongRef).url)
          val result  = await(route(application, request).value)
          events
            .collectFirst { case event =>
    ...
    "must redirect to site details page when loaded but no site ref exists in list on submit" in {
      ...
      running(application) {
        withCaptureOfLoggingFrom(application.injector.instanceOf[GenericLogger].logger) { events =>
          val request =
            FakeRequest(POST, routes.RemovePackagingSiteDetailsController.onSubmit(NormalMode, wrongRef).url)
          val result  = await(route(application, request).value)
          events
            .collectFirst { case event =>
              event.getLevel.levelStr mustBe "WARN"
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      val ref: String                      = "foo"
      val packagingSite: Map[String, Site] = Map(ref -> Site(UkAddress(List("a", "b"), "c"), None, aTradingName, None))
      val htmlExpectedInView               =
        Html(s"$aTradingName<br>a, b, <span class=\"nowrap\" style=\"white-space: nowrap;\">c</span>")
      ...
      running(application) {
        ...
        contentOfResult mustEqual view(boundForm, NormalMode, ref, htmlExpectedInView)(using
          request,
