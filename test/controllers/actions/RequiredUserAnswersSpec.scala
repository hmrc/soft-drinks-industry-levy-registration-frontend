...
class RequiredUserAnswersSpec extends SpecBase with DefaultAwaitTimeout {
  ...
  def dataRequest(userAnswers: UserAnswers): DataRequest[AnyContentAsEmpty.type] = DataRequest(
    FakeRequest(),
    "",
    hasCTEnrolment = false,
    None,
    userAnswers,
    RosmWithUtr("", RosmRegistration("", None, None, UkAddress(List.empty, "", None)))
  )
  "requireData" - {
    "should return result passed in when not a page matched in function" in {
      contentAsString(
        requiredUserAnswers.requireData(VerifyPage)(exampleSuccessAction)(using dataRequest(emptyUserAnswers))
      ) mustBe exampleSuccessActionResult
    }
      ...
      s"when HowManyLitresGlobally is $Large and all answers are answered" in {
        val userAnswers =
          emptyUserAnswers
            ...
            .success
            .value
            .set(HowManyOperatePackagingSitesPage, LitresInBands(1, 1))
            .success
            .value
            ...
            .success
            .value
            .set(StartDatePage, LocalDate.now())
            .success
            .value
            ...
            .value
        contentAsString(
          requiredUserAnswers.requireData(CheckYourAnswersPage)(exampleSuccessAction)(using dataRequest(userAnswers))
        ) mustBe exampleSuccessActionResult
      }
    }
      ...
      s"when $HowManyLitresGlobally is $None and all answers are answered" in {
        val userAnswers =
          emptyUserAnswers
            ...
            .success
            .value
            .set(HowManyLitresGloballyPage, HowManyLitresGlobally.None)
            .success
            .value
            .set(ContractPackingPage, true)
            .success
        ...
        s"is false, $ImportsPage is false, and $ContactDetailsPage has been answered" in {
          val userAnswers =
            emptyUserAnswers
              ...
              .success
              .value
              .set(ContactDetailsPage, ContactDetails("aaa", "aaa", "123", "a@a.com"))
              .success
              .value
          contentAsString(
            requiredUserAnswers.requireData(CheckYourAnswersPage)(exampleSuccessAction)(using dataRequest(userAnswers))
    ...
    s"should redirect to verify controller when missing answers for $CheckYourAnswersPage" - {
      "with no answers" in {
        val res = redirectLocation(
          requiredUserAnswers.requireData(CheckYourAnswersPage)(exampleSuccessAction)(using
            dataRequest(emptyUserAnswers)
          )
        )
        ...
        s"when $eachPage is filtered out from a full list of user answers set to true, the user is taken to that page that is required for ${HowManyLitresGlobally.Large}" in {
          val userAnswers =
            emptyUserAnswers
              ...
              .success
              .value
              .set(HowManyLitresGloballyPage, HowManyLitresGlobally.Large)
              .success
              .value
              .set(ContractPackingPage, true)
              .success
        ...
        s"when $eachPage is filtered out from a full list of user answers of true, the user is taken to that page that is required for ${HowManyLitresGlobally.Small}" in {
          val userAnswers =
            emptyUserAnswers
              ...
              .success
              .value
              .set(HowManyLitresGloballyPage, HowManyLitresGlobally.Small)
              .success
              .value
              .set(ContractPackingPage, true)
              .success
        ...
        s"when $eachPage is filtered out from a full list of user answers of false, the user is taken to that page that is required for ${HowManyLitresGlobally.Small}" in {
          val userAnswers =
            emptyUserAnswers
              ...
              .success
              .value
              .set(WarehouseDetailsPage, false)
              .success
              .value
              .set(ContactDetailsPage, ContactDetails("", "", "", ""))
              .success
  ...
  "returnMissingAnswers" - {
    ...
    "should return all but 1 missing answers when user answers is fully populated apart from 1 answer" in {
      ...
      res mustBe List(
        RequiredPage(
          ContactDetailsPage,
          List(PreviousPage(AskSecondaryWarehousesPage, List(true, false))(implicitly[Reads[Boolean]]))
      ...
      "is true and pack at business address is not answered" in {
        ...
        res mustBe List(
          RequiredPage(
            PackAtBusinessAddressPage,
            List(
              PreviousPage(HowManyLitresGloballyPage, List(HowManyLitresGlobally.enumerable.withName("large").get))(
                implicitly[Reads[HowManyLitresGlobally]]
              ),
              PreviousPage(OperatePackagingSitesPage, List(true))(implicitly[Reads[Boolean]]),
              PreviousPage(ContractPackingPage, List(true, false))(implicitly[Reads[Boolean]])
            )
        ...
        res mustBe List(
          RequiredPage(
            ...
            List(
              ...
              PreviousPage(ContractPackingPage, List(true))(implicitly[Reads[Boolean]])
            )
          )(implicitly[Reads[Boolean]])
        )
      }
        ...
        res mustBe List(
          RequiredPage(
            ...
            List(
              ...
              PreviousPage(ImportsPage, List(true))(implicitly[Reads[Boolean]])
            )
        ...
        res mustBe List(
          RequiredPage(
            ...
            List(
              ...
              PreviousPage(ImportsPage, List(false))(implicitly[Reads[Boolean]])
            )
          )(implicitly[Reads[ContactDetails]])
        )
      }
    ...
    "should return nothing when a list is provided for previous pages and previous pages don't exist" in {
      ...
      implicit val request: DataRequest[AnyContentAsEmpty.type] = dataRequest(emptyUserAnswers)
  ...
  "checkYourAnswersRequiredData" - {
    "should redirect to verify controller when missing answers" in {
      ...
      redirectLocation(
        requiredUserAnswers.checkYourAnswersRequiredData(exampleSuccessAction)
      ).get mustBe controllers.routes.VerifyController.onPageLoad(CheckMode).url
    }
