  ...
  "RegistrationConfirmation Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        when(
          mockSdilSessionCache.fetchEntry[CreatedSubscriptionAndAmountProducedGlobally](
            userAnswers.id,
        ...
        val headingAndSummaryItems =
          RegistrationSummary.summaryList(CreatedSubscriptionAndAmountProducedGlobally(subscription, Large), false)

        status(result) mustEqual OK
        contentAsString(result) mustEqual view(
          headingAndSummaryItems,
          submittedDateTime,
    ...
    "must redirect to start of journey" - {
      ...
      "if registration when the cache is empty" in {
        ...
        val application = applicationBuilder(
          userAnswers = Some(emptyUserAnswers.copy(submittedOn = Some(Instant.now))),
          utr = Some(rosmRegistration.utr)
        )
          .overrides(
            bind[SDILSessionCache].toInstance(mockSdilSessionCache),
            bind[SoftDrinksIndustryLevyConnector].toInstance(mockSdilConnector)
