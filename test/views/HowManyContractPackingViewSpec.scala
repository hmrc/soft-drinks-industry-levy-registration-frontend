    ...
    List(NormalMode, CheckMode).foreach { mode =>
      "when in " + mode +" mode" - {
        ...
        "should have the expected heading" in {
          document.getElementsByClass(Selectors.heading).text() mustBe Messages("howManyContractPacking.heading")
        }

        "should include a govuk body with the expected content" in {
          document.getElementsByClass(Selectors.body).text() mustBe Messages("howManyContractPacking.subtext")
        }
