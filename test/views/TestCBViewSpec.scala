...
import forms.TestCBFormProvider
import models.{CheckMode, NormalMode}
import play.api.i18n.Messages
import play.api.mvc.Request
import play.api.test.FakeRequest
import views.html.TestCBView

import models.TestCB

class TestCBViewSpec extends ViewSpecHelper {

  val view = application.injector.instanceOf[TestCBView]
  val formProvider = new TestCBFormProvider
  ...
  "View" - {
    ...
    "should contain the expected title" in {
      document.title() must include(Messages("testCB" + ".title"))
    }

    "should include a legend with the expected heading" in {
      val legend = document.getElementsByClass(Selectors.legend)
      legend.size() mustBe 1
      legend.get(0).getElementsByClass(Selectors.heading).text() mustEqual Messages("testCB" + ".heading")
    }
    ...
    "when the form is not preoccupied and has no errors" - {
      ...
      "should include the expected check boxes" - {
        ...
        "that has 2 items" in {
          checkboxes.size() mustBe TestCB.values.size
        }
        TestCB.values.zipWithIndex.foreach { case (checkbox, index) =>
          s"that has the " + checkbox.toString + " to select and is unchecked" in {
            ...
            checkbox1
              ...
              .text() mustBe Messages("testCB." + checkbox.toString)
            val input = checkbox1
    ...
    TestCB.values.foreach { checkbox =>
      ...
      s"when the form is preoccupied with " + checkbox.toString + "selected and has no errors" - {
        "should have checkboxes" - {
          ...
          TestCB.values.zipWithIndex.foreach { case (checkbox1, index) =>
            if (checkbox1.toString == checkbox.toString) {
              s"that has the option to select" + checkbox1.toString + " and is checked" in {
                ...
                checkboxes1
                  .getElementsByClass(Selectors.checkboxesLables)
                  .text() mustBe Messages("testCB." + checkbox1.toString)
    ...
    s"when the form is preoccupied with all checkboxes selected and has no errors" - {
      val htmlAllSelected = view(form.fill(TestCB.values.toSet), CheckMode)(request, messages(application))
    ...
    "contains a form with the correct action" - {
      "when in CheckMode" in {
        ...
        documentAllSelected.select(Selectors.form)
          .attr("action") mustEqual routes.TestCBController.onSubmit(CheckMode).url
      }

      "when in NormalMode" in {
        val htmlAllSelected = view(form.fill(TestCB.values.toSet), NormalMode)(request, messages(application))
        val documentAllSelected = doc(htmlAllSelected)

        documentAllSelected.select(Selectors.form)
          .attr("action") mustEqual routes.TestCBController.onSubmit(NormalMode).url
      }
    }
    ...
    "when there are form errors" - {
      ...
      "should have a title containing error" in {
        val titleMessage = Messages("testCB.title")
