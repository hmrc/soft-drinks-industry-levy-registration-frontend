...
import models.{Contact, PackagingSiteName}
import play.twirl.api.Html
import viewmodels.summary.{ContactDetailsSummary, PackagingSiteNameSummary}

class PackagingSiteNameSummarySpec extends RegistrationSubscriptionHelper {

  val NAME = "Site 123"

  val packagingSiteName: PackagingSiteName = PackagingSiteName(
    NAME
  )
    ...
    "should return the heading and summary list" - {
      ...
      "that contains change links" - {
        "when called for check your answers" in {
          val res = PackagingSiteNameSummary.row(subscription, true)
