...
class ContactDetailsSummarySpec extends SpecBase {
  ...
  val userContactDetails: ContactDetails = ContactDetails(
    NAME,
    POSITION,
    PHONE,
    EMAIL
  ...
  "headingAndSummary" - {
      ...
      "should return the heading and summary list" - {
        "that contains change links" - {
          "when called for check your answers" in {
            val res = ContactDetailsSummary.headingAndSummary(userAnswersWithContact, true)
            res mustBe defined
        ...
        "that does not contains change links" - {
          "when not called for check your answers" in {
            val res = ContactDetailsSummary.headingAndSummary(userAnswersWithContact, false)
            res mustBe defined
            val (heading, summaryList) = res.get
            heading mustBe "Contact person details"
            val summaryRows = summaryList.rows
    ...
    "when the userAnswers doesn't contain contact details" - {
      "should return none" - {
        "when called for check your answers" in {
          val res = ContactDetailsSummary.headingAndSummary(emptyUserAnswers)
          res mustBe None
        }

        "when not called for check your answers" in {
          val res = ContactDetailsSummary.headingAndSummary(emptyUserAnswers, false)
          res mustBe None
        }
      }
    }
