...
class PackagingSiteDetailsSummarySpec extends SpecBase {
  ...
  "row2" - {
    ...
    "should return an empty list of summaryListRows when no packaging site list is passed in" in {
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(Map.empty, NormalMode)

      packagingSiteSummaryRowList mustBe List()
    }

    "must not return a remove action if only 1 packaging site is passed in" in {
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(packagingSiteListWith1, NormalMode)
    ...
    "must include Correct elements in list with 2 elements" in {
      ...
      val site2 = Site(
        ...
        Some("trade"),
        None)
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(Map("ref1" -> site1, "ref2" -> site2), NormalMode)
      ...
      packagingSiteSummaryRowList.head.actions.toList.head.items.last.href mustBe routes.RemovePackagingSiteDetailsController.onPageLoad(NormalMode, "ref1").url
      ...
      packagingSiteSummaryRowList.last.actions.toList.head.items.last.href mustBe routes.RemovePackagingSiteDetailsController.onPageLoad(NormalMode, "ref2").url
    }
  }
  ...
  "address formatting within Row2" - {
    ...
    "should place a break after a trading name if a trading name is used" in {
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(Map(("90831480921", addressWith3AddressLines)), NormalMode)
    ...
    "should not place a break before the post code if the address line and post code length is 44 characters" in {
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(Map(("1208934391", address44Characters)), NormalMode)
    ...
    "should place a break before the post code if the address line and post code length is between 45 and 49 characters" in {
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(packagingSiteListWith3, NormalMode)
    ...
    "should not place a break before the post code if the address line and post code length is 50 characters" in {
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(Map(("3489028394r", address50Characters)), NormalMode)
    ...
    val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(Map(("45641351", PackagingSiteEvenLongerAddressNoTradeName)), NormalMode)
    ...
    "should place a break after a trading name AND autowrap and place a break before the post code if the address line " +
      ...
      val packagingSiteSummaryRowList = PackagingSiteDetailsSummary.row2(Map(("56458678", PackagingSiteEvenLongerAddressWithTradeName)), NormalMode)
