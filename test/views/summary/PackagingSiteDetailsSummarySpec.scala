...
class PackagingSiteDetailsSummarySpec extends SpecBase {
  ...
  val PackagingSite1 = Site(UkAddress(List("33 Rhes Priordy", "East London"), "E73 2RP"), None, aTradingName, None)
  ...
  val address44Characters =
    Site(UkAddress(List("29 Station Rd", "The Railyard", "Cambridge"), "CB1 2FP"), Some("10"), aTradingName, None)

  val address45Characters =
    Site(UkAddress(List("29 Station Pl.", "The Railyard", "Cambridge"), "CB1 2FP"), None, aTradingName, None)

  val address47Characters =
    Site(UkAddress(List("29 Station Place", "The Railyard", "Cambridge"), "CB1 2FP"), Some("10"), aTradingName, None)

  val address49Characters =
    Site(UkAddress(List("29 Station PlaceDr", "The Railyard", "Cambridge"), "CB1 2FP"), None, aTradingName, None)

  val address50Characters =
    Site(UkAddress(List("29 Station Place Dr", "The Railyard", "Cambridge"), "CB1 2FP"), Some("10"), aTradingName, None)
    ...
    s"In ${mode.toString}" - {
      "row2" - {
        ...
        "must include Correct elements in list with 2 elements" in {
          val site1                       = Site(UkAddress(List("foo2", "bar2"), "wizz2"), None, "trade2", None)
          val site2                       = Site(UkAddress(List("foo", "bar"), "wizz"), None, "trade", None)
          val packagingSiteSummaryRowList =
            PackagingSiteDetailsSummary.row2(Map("ref1" -> site1, "ref2" -> site2), mode)
          packagingSiteSummaryRowList.head.key.content.asHtml
            .toString() mustBe "trade2<br>foo2, bar2, <span class=\"nowrap\" style=\"white-space: nowrap;\">wizz2</span>"
          packagingSiteSummaryRowList.head.actions.toList.head.items.last.content.asHtml.toString() mustBe "Remove"
          packagingSiteSummaryRowList.head.actions.toList.head.items.last.href mustBe routes.RemovePackagingSiteDetailsController
            .onPageLoad(mode, "ref1")
            .url

          packagingSiteSummaryRowList.last.key.content.asHtml
            .toString() mustBe "trade<br>foo, bar, <span class=\"nowrap\" style=\"white-space: nowrap;\">wizz</span>"
          packagingSiteSummaryRowList.last.actions.toList.head.items.last.content.asHtml.toString() mustBe "Remove"
          packagingSiteSummaryRowList.last.actions.toList.head.items.last.href mustBe routes.RemovePackagingSiteDetailsController
      ...
      "address formatting within Row2" - {
        ...
        "should place a break after a trading name if a trading name is used" in {
          val packagingSiteSummaryRowList =
            PackagingSiteDetailsSummary.row2(Map(("90831480921", addressWith3AddressLines)), mode)
          val expectedAddressContent      = HtmlContent(
            "Test trading name 1<br>The house, The Road, ugzhkxcajkcjfrqsgkjruzlmsxytwhg vdg, <span class=\"nowrap\" style=\"white-space: nowrap;\">NW88 8II</span>"
          )
        ...
        "should not place a break before the post code if the address line and post code length is 44 characters" in {
          val packagingSiteSummaryRowList =
            PackagingSiteDetailsSummary.row2(Map(("1208934391", address44Characters)), mode)
          val expectedAddressContent      = HtmlContent(
        ...
        "should not place a break before the post code if the address line and post code length is 50 characters" in {
          val packagingSiteSummaryRowList =
            PackagingSiteDetailsSummary.row2(Map(("3489028394r", address50Characters)), mode)
          val expectedAddressContent      = HtmlContent(
        ...
        "should autowrap and place a break before the post code if the address line and post code length is between 98 & 103 characters" in {
          val packagingSiteSummaryRowList =
            PackagingSiteDetailsSummary.row2(Map(("45641351", PackagingSiteEvenLongerAddressNoTradeName)), mode)
          val expectedAddressContent      = HtmlContent(
            s"$aTradingName<br>29 Station Rd, This address will auto wrap but not in postcode," +
        ...
        "should place a break after a trading name AND autowrap and place a break before the post code if the address line " +
          "and post code length is between 98 & 103 characters" in {
            val packagingSiteSummaryRowList =
              PackagingSiteDetailsSummary.row2(Map(("56458678", PackagingSiteEvenLongerAddressWithTradeName)), mode)
            val expectedAddressContent      = HtmlContent(
