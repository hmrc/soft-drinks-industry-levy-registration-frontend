...
class EnterBusinessDetailsViewSpec extends ViewSpecHelper {
  ...
  "View" - {
    ...
    "should contain a govuk form group" - {
      ...
      "that contains the expected hint test" in {
        formGroup
          ...
          "This is 10 numbers, for example 1234567890. It will be on tax returns and other letters about Corporation Tax. It may be called ‘reference’, ‘UTR’ or ‘official use’."
        )
      }
    ...
    "contains a form with the correct action" in {
      val htmlAllSelected     =
        view(form.fill(Identify("testing", "testing")))(using request, messages(application), frontendAppConfig)
    ...
    "when a form error exists (utr invalid characters + no postcode)" - {
      ...
      val htmlWithErrors     =
        view(form.bind(Map("utr" -> valueOutOfMaxRange)))(using request, messages(application), frontendAppConfig)
