...
import forms.TestYNWithDetailsFormProvider
import models.{CheckMode, NormalMode}
import play.api.i18n.Messages
import play.api.mvc.Request
import play.api.test.FakeRequest
import views.html.TestYNWithDetailsView

class TestYNWithDetailsViewSpec extends ViewSpecHelper {

  val view = application.injector.instanceOf[TestYNWithDetailsView]
  val formProvider = new TestYNWithDetailsFormProvider
  ...
  "View" - {
    ...
    "should contain the expected title" in {
      document.title() must include(Messages("testYNWithDetails" + ".title"))
    }

    "should include a legend with the expected heading" in {
      val legend = document.getElementsByClass(Selectors.legend)
      legend.size() mustBe 1
      legend.get(0).getElementsByClass(Selectors.heading).text() mustEqual Messages("testYNWithDetails.heading")
    }
    ...
    val expectedDetails = Map(Messages("testYNWithDetails.detailsLink") -> Messages("testYNWithDetails.detailsContent"))
    ...
    "contains a form with the correct action" - {
      "when in CheckMode" - {
        ...
        "and yes is selected" in {
          documentYesSelected.select(Selectors.form)
            .attr("action") mustEqual routes.TestYNWithDetailsController.onSubmit(CheckMode).url
        }
      ...
      "when in NormalMode" - {
        ...
        "and yes is selected" in {
          documentYesSelected.select(Selectors.form)
            .attr("action") mustEqual routes.TestYNWithDetailsController.onSubmit(NormalMode).url
        }
    ...
    "when there are form errors" - {
      ...
      "should have a title containing error" in {
        val titleMessage = Messages("testYNWithDetails.title")
      ...
      "contains a message that links to field with error" in {
        ...
        errorSummary.text() mustBe Messages("testYNWithDetails.error.required")
      }
    }
