...
class TestDateViewSpec extends ViewSpecHelper {

  val view = application.injector.instanceOf[TestDateView]
  val formProvider = new TestDateFormProvider
  ...
  "View" - {
    ...
    "should contain the expected title" in {
      document.title() must include(Messages("testDate" + ".title"))
    }

    "should include a legend with the expected heading" in {
      val legend = document.getElementsByClass(Selectors.legend)
      legend.size() mustBe 1
      legend.get(0).getElementsByClass(Selectors.heading).text() mustEqual Messages("testDate" + ".heading")
    }
    ...
    "contains a form with the correct action" - {
      "when in CheckMode" in {
        ...
        documentAllSelected.select(Selectors.form)
          .attr("action") mustEqual routes.TestDateController.onSubmit(CheckMode).url
      }
      ...
      "when in NormalMode" in {
        ...
        documentAllSelected.select(Selectors.form)
          .attr("action") mustEqual routes.TestDateController.onSubmit(NormalMode).url
      }
    }
    ...
    "when there are form errors" - {
      ...
      "should have a title containing error" in {
        val titleMessage = Messages("testDate.title")
