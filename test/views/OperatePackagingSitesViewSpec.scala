...
class OperatePackagingSitesViewSpec extends ViewSpecHelper {

  val view = application.injector.instanceOf[OperatePackagingSitesView]
  val formProvider = new OperatePackagingSitesFormProvider
  ...
  "View" - {
    ...
    "should contain the expected title" in {
      document.title() must include(Messages("operatePackagingSites" + ".title"))
    }

    "should include a legend with the expected heading" in {
      val legend = document.getElementsByClass(Selectors.legend)
      legend.size() mustBe 1
      legend.get(0).getElementsByClass(Selectors.legend).text() mustEqual Messages("Do you operate any packaging sites in the UK to package liable drinks for the brands you own?")
    }
    ...
    "contains a form with the correct action" - {
      "when in CheckMode" - {
        ...
        "and yes is selected" in {
          documentYesSelected.select(Selectors.form)
            .attr("action") mustEqual routes.OperatePackagingSitesController.onSubmit(CheckMode).url
        }
      ...
      "when in NormalMode" - {
        ...
        "and yes is selected" in {
          documentYesSelected.select(Selectors.form)
            .attr("action") mustEqual routes.OperatePackagingSitesController.onSubmit(NormalMode).url
        }
    ...
    "when there are form errors" - {
      ...
      "should have a title containing error" in {
        val titleMessage = Messages("operatePackagingSites.title")
