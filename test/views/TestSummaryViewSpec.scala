...
import views.html.TestSummaryView

class TestSummaryViewSpec extends ViewSpecHelper {

  val view = application.injector.instanceOf[TestSummaryView]
  ...
  "View" - {
    ...
    "should contain the expected title" in {
      document.title() must include(Messages("testSummary" + ".title"))
    }

    "should include the expected panel" in {
      val panel = document.getElementsByClass(Selectors.panel).get(0)
      panel.getElementsByClass(Selectors.panel_title).text() mustEqual Messages("testSummary" + ".title")
      panel.getElementsByClass(Selectors.panel_body).text() mustEqual Messages("testSummary" + ".panel.message")
    }
    ...
    "should include a what happens next section" - {
      ...
      "that has the expected body" in {
        ...
        body.text() mustEqual Messages("testSummary.whatNextText")
      }
    }
    ...
    "should include a details section" - {
      ...
      "that has the expected details summary text" in {
        details.getElementsByClass(Selectors.detailsText).text() mustEqual Messages("testSummary.detailsSummary")
      }
